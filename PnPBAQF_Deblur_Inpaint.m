function x= PnPBAQF_Deblur_Inpaint(opts, x0, y, ker,M,H) %% Pixel value ranges in [0,255]
...
x =y ; %%% Initilization
x_real=q2double(x);
d1 =double2q(zeros(m,n,3));   d3 = double2q( zeros(m,n,3)) ;  
d2 = double2q(zeros(m,n,3));
%%%***************************************************************
...
%%% z2-subproblem  
%%%%%%%%%%%%%%%%%%%%%%%%%%
Z2 = x - d2/mu2 ; Sigma = sqrt(lambda2/mu2) ;
Z2 = q2double(Z2);
...
z3 = (mu3*(Ax-d3/mu3) + MTy)./(mu3 + M);   

%%%%%%%%%%%%%%%%%%%%%%%%%%% 
%%% x-subproblem
%%%%%%%%%%%%%%%%%%%%%%%%%%%
x_prev = x ;
x=(IFT((mu1.*FT(z1+d1/mu1)+mu2.*FT(z2+d2/mu2)+mu3.*real(fft2(H)).*fft2(z3+d3/mu3))./D_));
x_real=q2double(real(x));
x_real(x_real>255) = 255; x_real(x_real<0) = 0;  %% range [0 255]    
x = double2q(x_real);
